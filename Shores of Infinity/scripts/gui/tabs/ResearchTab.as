...
class ResearchEditor : ResearchTab {
	...
	void update() {
		if(!created) {
			...
			const string fname = resolve("data/research/excl/"+filename);
	...
	void save() {
		const string fname = path_join(topMod.abspath, "data/research/excl/"+filename);
		ensureFile(fname);
...
class ResearchEditorCommand : ConsoleCommand {
	void execute(const string& args) {
		...
		if(args.length == 0) {
			fname = "standard_grid.txt";
			ident = "StandardResearch";
		}
		else {;
