...
final class Infrastructure : AIComponent {
			...
			if (next !is null)
			{
				...
				switch (next.action) {
					...
					case BA_BuildStarport:
						switch (next.loc) {
							case BL_InSystem:
								sys.buildInSystem(this, ai.defs.Starport, next.priority, next.force);
								break;
							case BL_AtSystemEdge:
								sys.buildAtSystemEdge(this, ai.defs.Starport, next.priority, next.force);
								break;
							case BL_AtBestPlanet:
									@obj = getBestPlanet(sys);
									if (obj !is null) {
										sys.buildAtPlanet(this, cast<Planet>(obj), ai.defs.Starport, next.priority, next.force);
									}
								break;
							default:
	...
	bool shouldHaveStarport(SystemCheck& sys, SystemArea area, SystemBuildLocation&out loc) const {
		loc = BL_InSystem;
	...
	bool shouldHaveMoonBase(PlanetCheck& pl) const {
		...
		/*if (type !is null && (type.cls is scalableClass || type.level == 3 || type.level == 2) && pl.ai.obj.emptyDevelopedTiles < 9)
			return true;*/
