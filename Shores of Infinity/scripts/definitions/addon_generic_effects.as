...
class AddStatusStacks : GenericEffect {
	...
	Argument multiplier_status(AT_Status, doc="Status to check for, if present, a percentage increase to the occurrence of triggers (decrease to interval), eg. 0.1 for +10%. Requires a variable status effect.");
	...
	Argument random_interval_margin(AT_Decimal, "0", doc="Random margin for the interval. Not used if there is only one trigger.");
	...
	#section server
		...
		void tick(Object& obj, any@ data, double tick) const override {
			if (interval.decimal > 0) {
				...
				double factor = 0.0;
				if (multiplier_status.integer > -1) {
					for(uint i = 0, cnt = obj.statusEffectCount; i < cnt; ++i) {
						if (obj.statusEffectType[i] == uint(multiplier_status.integer)) {
							factor = obj.variable[i];
							break;
						}
					}
				}

				dat.timer -= tick * (1 + factor);
				if (dat.timer <= 0.0) {
					addStatusStacks(obj);
					dat.count += 1;
					dat.timer = getTimer();
