...
class RequirePlanet : Requirement {
	...
	Argument settleable(AT_Boolean, "False", doc="Also require that the planet has at least one settleable biome.");

	bool meets(Object& obj, bool ignoreState = false) const override {
		return !settleable.boolean ? obj.isPlanet : obj.isPlanet && canSettle(cast<Planet>(obj));
	}

	bool canSettle(Planet& planet) {
		for (uint i = 0, cnt = planet.biomeCount; i < cnt; ++i) {
			if (getBiome(planet.biomeId[i]).settleable)
				return true;
		}
		return false;
	}
