...
class OnTradeDeliver : ResourceHook {
	...
	Document doc("The inner hook is triggered when a civilian ship carrying this resource delivers its cargo. If system flags are specified for the source or the destination or both, the ineer hook is triggered only if those flags are present.");
...
#section server
	void onTradeDeliver(Civilian& civ, Object@ origin, Object@ target) const override {
		...
		if (source_Flag.str == noFlag)
			...
			if (origin !is null && !origin.isRegion) {
				@reg = origin.region;
				if(reg !is null)
					sourceFlag = reg.getSystemFlag(origin.owner, source_Flag.integer);
			}
		}
		...
		if (destination_Flag.str == noFlag)
			...
			if (target !is null && !target.isRegion) {
				@reg = target.region;
				if(reg !is null)
					destFlag = reg.getSystemFlag(target.owner, destination_Flag.integer);
			}
		}
